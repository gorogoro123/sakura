...
//2002.02.08 Grepアイコンも大きいアイコンと小さいアイコンを別々にする。
	...
	::SetWindowLongPtr( hFolder, GWLP_USERDATA, (LONG_PTR)this );
	m_pOnFolderProc = (WNDPROC)::SetWindowLongPtr( hFolder, GWLP_WNDPROC, (LONG_PTR)OnFolderProc );
...
*/
LRESULT CALLBACK CDlgGrep::OnFolderProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	auto pcDlgGrep = (CDlgGrep *)::GetWindowLongPtr( hwnd, GWLP_USERDATA );
	...
	case WM_DESTROY:
		::SetWindowLongPtr( hwnd, GWLP_WNDPROC, (LONG_PTR)pcDlgGrep->m_pOnFolderProc );
		pcDlgGrep->m_pOnFolderProc = nullptr;
		return 0;
	default:
		break;
	}

	return  CallWindowProc( pcDlgGrep->m_pOnFolderProc, hwnd, msg, wparam, lparam );
}
