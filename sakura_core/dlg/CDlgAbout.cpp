...
BOOL CDlgAbout::OnInitDialog( HWND hwndDlg, WPARAM wParam, LPARAM lParam )
{
	...
	cmemMsg.AppendStringF(
		L"%s Ver. %d.%d.%d.%d " TEXT(BUILD_ENV_NAME) TEXT(VERSION_HASH) L"\r\n",
		LS(STR_GSTR_APPNAME),
	...
	// コンパイル情報
	cmemMsg.AppendStringF(
		L"      Compile Info: " COMPILER_TYPE TARGET_M_SUFFIX L"%d " TSTR_TARGET_MODE L" WIN%03x/I%03x/C%03x/N%03x\r\n",
...
//	case IDC_STATIC_URL_ORG:	del 2008/7/4 Uchi
	...
	case IDC_STATIC_URL_CI_BUILD:
		{
#if defined(CI_BUILD_URL)
			::ShellExecute(GetHwnd(), NULL, TEXT(CI_BUILD_URL), NULL, NULL, SW_SHOWNORMAL);
#elif defined(GIT_REMOTE_ORIGIN_URL)
			::ShellExecute(GetHwnd(), NULL, TEXT(GIT_REMOTE_ORIGIN_URL), NULL, NULL, SW_SHOWNORMAL);
#endif
			return TRUE;
		}
	case IDC_STATIC_URL_GITHUB_COMMIT:
#if defined(GITHUB_COMMIT_URL)
		::ShellExecute(GetHwnd(), NULL, TEXT(GITHUB_COMMIT_URL), NULL, NULL, SW_SHOWNORMAL);
#endif
		return TRUE;
	case IDC_STATIC_URL_GITHUB_PR:
#if defined(GITHUB_PR_HEAD_URL)
		::ShellExecute(GetHwnd(), NULL, TEXT(GITHUB_PR_HEAD_URL), NULL, NULL, SW_SHOWNORMAL);
#endif
		return TRUE;
	}
	/* 基底クラスメンバ */
